...
export default function MyPostsPage() {
  ...
  // טעינת כל הפוסטים של המשתמש המחובר
  const loadMyPosts = async () => {
    try {
      ...
      if (!token) {
        setIsLoggedIn(false);
        setMessage("You must be logged in to see your posts");
        setIsLoading(false);
        return;
      }
      ...
      if (!response.ok) {
        setMessage(data.message || "Could not fetch your posts");
        return;
      }
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen bg-[#f8f3ed] flex items-center justify-center">
        <p className="text-[#5f4b4b] text-lg">Loading your posts...</p>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#f8f3ed] px-6 py-12">
      <section className="mx-auto max-w-4xl">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-[#3b2f2f]">My Posts</h1>

          <p className="mt-3 text-[#5f4b4b]">
            All posts you have published across your reading groups.
          </p>
        </div>
        ...
        {!message && posts.length === 0 && (
          <div className="rounded-2xl border border-[#eadfd4] bg-white p-8 text-center">
            ...
            <p className="mt-3 text-[#5f4b4b]">
              Go to one of your groups and create your first post.
            </p>
          ...
          <div className="flex flex-col items-center gap-5">
            {posts.map((post) => (
              ...
              >
                ...
                <div className="mt-4 text-sm text-[#8a7474]">
